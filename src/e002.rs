...
//! including borrowing!
//!
//! After taking a short look at one of Rust's basic approaches to creating new types, we dive into a *fairly* thorough
//! overview of how borrowing works in fairly run-of-the-mill Rust code. This is a basic introduction, and as such I'm
//! not getting into things like heap-allocated memory (`Box`) or dealing with `move` semantics with threads or
//! closures. (I haven't actually figured those out well enough yet to write something like this for them!)
//!
...
//! [2]: https://www.ncameron.org/blog/rustfmt-ing-rust/
...
pub fn demonstrate_ownership() {
    ...
    // If I move an object, I can no longer access it afterward. If you uncomment the following line, you'll see that
    // the Circle instances are no longer accessible. Note that moving and borrowing are orthogonal to mutability. It's
...
///
/// This lets use demonstrate both how methods work in general and specifically how they interact with the idea of
/// ownership.
impl Circle {
    ...
    /// Returns the value of `Circle.x`, borrowing a mutable reference to the circle and changing the value
    /// (demonstrating a situation in which you would want to use a mutable rather than immutable reference).
    ...
    /// Returns the value of `Circle.x`, taking ownership of the circle. As a result of the change in ownership, the
    /// circle goes out of scope after the method returns, so the circle instance will be inaccessible after that.
    ///
    /// Note that the item is taken as immutable, so attempting to change the internals will still fail. **Ownership is
...
pub fn demonstrate_method_ownership() {
    // There are several ways to construct struct types. The first is a plain struct constructor.
    let basic = Circle {
        x: 1.0,
        y: 2.0,
        r: 5.0,
    ...
    // Now we can see how different method calls work. When we borrow a reference, there is no consequence: during the
    // life of the method, the immutable object is "borrowed" by the `x_by_ref` method. Ownership returns to the `main`
    // function when it returns.
    let by_ref_x = immutable.x_by_ref();
    println!("By reference: {}", by_ref_x);

    // The same is true of a method that borrows a mutable reference. Note, however, that since the reference is
    // mutable, it can change the contents of the struct.
    ...
    // Note that both of these struct instances are still available for `main` to use once they return from the method
    // call:
    println!(
    ...
    // A method that takes ownership can still return a value. However, it has a very different behavior in terms of
    // the struct instance, in a way that seems surprising compared to languages without the concept of "borrowing" vs.
    // "taking" (C, Java, Python, etc.).
    //
    // Run a method that takes ownership rather than just borrowing a reference. The value returned is as expected.
    // However, the item has been *moved*, not just borrowed. That is, `main` no longer owns it---`by_take` took over
    // the ownership. Once `by_take` ended, `immutable` actually gets deconstructed ("dropped") because it went out of
    // scope. As a result, the following line won't compile (you can uncomment it to see for yourself):
    let by_take_x = immutable.by_take();
    println!("By take: {}", by_take_x);
    // println!("Does the circle still exist? {:?}", immutable);

    // Likewise, if we use the mutable circle with the method which takes ownership and mutates it, we can get the
    // value back as before, but the circle will have been dropped when the `by_take_mut` call ended.
