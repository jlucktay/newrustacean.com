...
//!
//! The functions themselves are not especially interesting; they just show you the basic form you use to return
//! functions from other functions, and how to then use them in another function. You'll want to take a detailed look
...
//!     memory for the smart pointer and associated metadata are allocated on the stack. I had this in mind, and sort
//!     of alluded to it earlier in the discussion, but the way I actually said it was misleading at best.
//!
//!   - Chad Sharp (@crossroads1112 on GitHub) [clarified][notes-3] that in C99, C *does* support dynamic array
//!     allocation, though it became optional for compilers to implement it as of C11. I forgot about this because I
//!     spend so much of my time dealing with Visual C++, which does not support dynamic array allocation. (Notably,
//!     Visual C does; it is Visual C++ which does not: remember, C and C++ are related but distinct languages.)
//!
...
///
/// Note that it doesn't have `Fn(i32) -> i32` as its return type, but rather `Box<Fn(i32) -> i32`. At present,
/// functions have to be explicitly heap-allocated. If we tried to return just a `Fn` type, we'd end up in a tangled
/// mess of lifetime and ownership (whether with a regular function, as here, or a closure as in
...
///
/// Compare with the function above: this is basically identical to `doubler_factory`, the only difference being that
/// in this case, we return a closure rather than a standalone function. That has substantial ramifications for using
/// closures and named functions, of course; refer to the discussion in [e004][e004] for details.
///
...
/// ```
///
/// And another. This gets us a bit closer, because you actually have both a concrete reference type and a lifetime.
/// However, it still won't compile, because after `get_normal_function_with_lifetime` ends, the function goes out of
/// scope and the reference points to junk value.
///
/// ```rust,ignore
...
/// ```
///
/// Even this approach doesn't work, because although the *function* lives on past the end of the
/// `get_normal_external_fn_with_lifetime`, the reference created during that function call *doesn't*.
///
/// ```rust,ignore
