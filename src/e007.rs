...
//!
//! The detailed code samples for this episode are heavy on showing; because of the nature of test functions, you will
//! be best off just [reading the source] rather than leaning heavily on the descriptions generated by **rustdoc**.
//! (The descriptions are still *there*, but they're much less useful than they have been in previous episodes.) In
//! particular, the `test` module here is excluded because of the use of the `#[cfg(test)]` attribute marker on it.
//!
//! [reading the source]: ../../src/show_notes/e007.rs.html
//!
//! Because we are using the feature-gated benchmarking functionality, the show notes "library" can now only be
//! compiled with the Rust nightly (as of 1.5, the version current as this episode is produced).
//!
//! One thing that isn't necessarily obvious from reading the test documentation in the Rust book and Rust reference:
...
///
/// This test function will not be compiled into a binary; it will *only* be used with the test binary. Note that you
/// *can* create standalone test functions as long as they are marked with the `#[test]` attribute, but in general, you
/// should put it in the `test` module, see below.
#[test]
fn test_add() {
...
#[cfg(test)]
mod tests {
    // This statement gives us access to everything in the parent module, so we don't have to use the full namespace
    // (`show_notes::e007::add`) to get access to the functions we want to test.
    use super::*;
    ...
    ///
    /// In a more meaningful scenario, we might use the `#[should_panic]` attribute to verify that a given function
    /// call triggers an error under conditions where it should, e.g. if we tried to `.unwrap` a value which didn't
    /// exist.
    #[test]
    #[should_panic]
    fn test_add_badly() {
    ...
    ///
    /// > ...it's hard to guarantee that the test didn't fail for an unexpected reason...
    ///
    ...
    ///
    /// Note: it's trivial, so it's probably pretty quick (`0 ns/iter (+/- 0)`). The point is simply that it does what
    /// it says on the tin.
    #[bench]
    ...
    ///
    /// This particular function is *stupid*; the way to do this, of course, is just to get the Duration directly. The
    /// only reason to have it here is to show that (1) `support_function()` doesn't end up in the compiled library,
    /// which you can check by inspecting the binary; and (2) that it is available for use with the benchmarker below.
    ...
    ///
    /// One of the things this highlights: we have a *tiny* duration (10 ns)... and the test takes much, *much* longer.
    /// (I'm going to discuss this with the Rust community, because I don't actually understand it yet!)
    #[bench]
